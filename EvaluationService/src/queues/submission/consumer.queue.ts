...
export const startSubmissionConsumer = () => {
  console.log("Starting submission consumer...");
  const worker = new Worker(
    ...
    async (job) => {
      ...
      return asyncLocalStorage.run({ correlationId: correlationId || 'unknown-job' }, async () => {

        console.log(`Received Data for ${isRunOnly ? 'Run' : 'Submission'} ${submissionId || job.id}`, { data: job.data });
        ...
        console.log(`Result for ${isRunOnly ? 'Run' : 'Submission'} ${submissionId || job.id}`, { result });
          ...
          if (contestId && submissionCreatedAt) {
            ...
            if (contestResponse.ok) {
              ...
              if (contest) {
                ...
                console.log(`Contest submission penalty calculated: ${timeTakenInMs}ms, EndTime: ${contestEndTime}`);
              }
            }
          }
        }
        ...
        try {
          ...
          if (!persistResponse.ok) {
            ...
            console.log(`[EVALUATION] Submission ${submissionId} persisted successfully.`);
          }
        } catch (err: any) {
        ...
        if (result.verdict === "AC") {
          ...
          if (contestId) {
            ...
            if (isFirstSolve === 0) {
              ...
              console.log(`[EVALUATION] Problem ${problemId} already solved by ${userId}. Skipping leaderboard update.`);
          ...
          if (shouldUpdateLeaderboard) {
            ...
            console.log(`[EVALUATION] CONTEST AC: Points: ${score}, Penalty: ${penaltyMinutes} min, RedisScore: ${finalEncodedScore.toFixed(6)}`);
          } else if (!contestId) {
            console.log(`[EVALUATION] PRACTICE AC: User ${userId} solved problem ${problemId}`);
          }
        }
  ...
  console.log("Submission consumer started successfully.");
