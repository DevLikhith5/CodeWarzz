...
export class SubmissionRepository {
    ...
    async getUserSolvedProblemIds(contestId: string, userId: string) {
        return await observeDbQuery('getUserSolvedProblemIds', 'submissions', async () => {
            const results = await db.select({ problemId: submissions.problemId })
                .from(submissions)
                .where(and(
            ...
            return new Set(results.map(r => r.problemId));
        });
    }
}
