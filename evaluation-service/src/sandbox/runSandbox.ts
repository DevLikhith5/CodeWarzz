...
export async function runSandbox({
  ...
  logger.info(`[SANDBOX] Starting execution for ${language} with ${testcases.length} test cases.`);
  ...
  try {
    ...
    try {
      ...
      logger.info(`[SANDBOX] Compilation Error`);
      return {
        verdict: "CE",
        passed: 0,
...
for i in $(seq 0 ${testcases.length - 1}); do
  ...
  ${earlyExitLogic}
    ...
    try {
      ...
      logger.info(`[SANDBOX] Batch execution verdict: ${batchVerdict}`);
    }
    ...
    logger.debug(`[SANDBOX] Analyzing results after ${totalTimeTakenMs}ms`);
    ...
    const finalVerdict = firstFailedResult?.verdict || "AC";
    logger.info(`[SANDBOX] Execution complete. Verdict: ${finalVerdict}, Passed: ${passed}/${testcases.length}`);
