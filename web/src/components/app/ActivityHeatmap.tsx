    ...
    return (
        <div className="text-xs">
                ...
                {monthLabels.map((m, i) => {
                    return (
                        <div
                            ...
                            style={{
                                left: `calc(2.25rem + ${m.weekIndex * 14}px)` // 2.25rem = pl-9 (36px)
                            }}
                        >
                            {m.label}
                        </div>
            ...
            <div className="flex gap-[2px] mt-4">
                {/* Day Labels */}
                <div className="flex flex-col gap-[2px] mr-1 w-8 text-[9px] text-muted-foreground justify-around pb-2">
                    <span>Mon</span>
                    <span>Wed</span>
                    <span>Fri</span>
                </div>
                    ...
                    <div key={idx} className="flex flex-col gap-[2px]">
                        {week.map((day, dIdx) => (
                            day ? (
                                ...
                                <div key={`empty-${dIdx}`} className="w-3 h-3" />
                            )
                        ))}
                    </div>
                ))}
